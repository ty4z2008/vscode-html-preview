...
function activate(context) {
    let htmlShowPreview = vscode.commands.registerCommand('html.realtimePreview', () => {
        ...
        let ishtml = utils_1.isHTML(activeTextEditor);
        if (!ishtml)
            return;
...
class RealtimePreview {
    constructor(context, panel, activeTextEditor) {
        this._disposables = [];
        this._panel = panel;
    ...
    bindEvent() {
        ...
        watcher.onDidChange(debounce(e => {
        ...
        this._panel.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            switch (message.command) {
                case 'setView':
                    ...
                    if (utils_1.isSupportFile(fileUrl)) {
                        this._fileUrl = this._panel.webview.asWebviewUri(fileUrl);
                    }
                    else {
                        this._fileUrl = yield vscode.env.asExternalUri(vscode.Uri.parse(fileUrl));
                    }
                    this._update();
                    return;
                case 'message':
     ...
     * createOrShow
     */
    static createOrShow(context, activeTextEditor) {
        ...
        const panel = vscode.window.createWebviewPanel('realtimePreview', 'Realtime Preview', vscode.ViewColumn.Beside, {
            enableScripts: true,
            enableFindWidget: true,
            localResourceRoots: [
                vscode.Uri.file(path_1.join(extensionPath, 'public')),
                vscode.Uri.file(vscode.workspace.rootPath)
            ]
        });
        RealtimePreview.currentPanel = new RealtimePreview(context, panel, activeTextEditor);
    }
    addRestStyles(extensionPath, html) {
        let styleLocalpath = vscode.Uri.file(path_1.join(extensionPath, 'public', 'reset.css'));
        const { webview } = this._panel;
    ...
    getWebviewContent(extensionPath, activeTextEditor) {
...
RealtimePreview.viewType = 'realtimePreview';
function deactivate() { }
